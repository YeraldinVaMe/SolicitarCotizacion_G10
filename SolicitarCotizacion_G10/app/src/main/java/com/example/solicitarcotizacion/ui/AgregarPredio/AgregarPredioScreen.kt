...
@Composable
fun AgregarPredioS(navController: NavController, viewModel: AgregarPredioViewModel, masterViewModel: MasterViewModel){
    ...
    Box {
        ...
        insertarPredioDialog(viewModel = viewModel, navController = navController)
        insertarPredioDialog2(viewModel = viewModel, navController = navController)
    }
}
...
fun AgregarPredioView(){
    MaterialTheme {
        AgregarPredioS(rememberNavController(),AgregarPredioViewModel(), MasterViewModel())
    }
}
        ...
        CardSolicitar001 {
            LazyColumn(contentPadding = PaddingValues(10.dp),
                ...
                item {
                    ...
                    BtnAgregarPredio(onBtnAgregarPredio = { viewModel.agregarPredio() }) {
                        Group45 {
                            AgregarPredio(
...
@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun tipoPred(viewModel: AgregarPredioViewModel){
    var expanded by remember { mutableStateOf(false) }
    val items = viewModel.tipoPredioResult.value
    ...
    DropdownMenu(
        ...
        items.forEach { tipoPredio ->
            DropdownMenuItem(onClick = {
                selectedText = tipoPredio.nomre_predio
                expanded = false
                viewModel.tipPredioSelected = tipoPredio.id_tipo_predio
            }) {
                Text(text = tipoPredio.nomre_predio)
            }
        }
    }
}
...
@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun codigoPostal(viewModel: AgregarPredioViewModel){
    var expanded by remember { mutableStateOf(false) }
...
@Composable
fun insertarPredioDialog(viewModel: AgregarPredioViewModel, navController: NavController){

    val show : Boolean by viewModel.showDialog.observeAsState(initial = false)

    if(show){
        AlertDialog(
            onDismissRequest = { navController.navigate(Screen.AgregarAreasComunes.route) },
            confirmButton = {
                TextButton(onClick = { navController.navigate(Screen.AgregarAreasComunes.route) }) {
                    Text(text = "Continuar")
                }
            },
            title = { Text(text = "Predio agregado!") },
            text = { Text(text = "Se registró correctamente el predio") },
        )
...
@Composable
fun insertarPredioDialog2(viewModel: AgregarPredioViewModel, navController: NavController){

    val show : Boolean by viewModel.showDialog2.observeAsState(initial = false)

    if(show){
        AlertDialog(
            onDismissRequest = { viewModel.reiniciarDialogos()  },
            confirmButton = {
                TextButton(onClick = { viewModel.reiniciarDialogos() }) {
                    Text(text = "Cerrar")
                }
            },
            title = { Text(text = "Error!") },
            text = { Text(text = "No se pudo registrar el predio") },
        )
