...
@Composable
fun SolicitarCotizacion(navController: NavController, viewModel: SolicitarCotizacionViewModel, masterViewModel: MasterViewModel) {
    val image = painterResource(R.drawable.fondo2)

    val nombre : String by masterViewModel.personaName.observeAsState(initial = "")
...
fun SolicitarCotizacionPreview(){
    SolicitarCotizacionTheme {
        SolicitarCotizacion(rememberNavController(),SolicitarCotizacionViewModel(), MasterViewModel())
    }
}
...
fun CardPrincipalv1(
    ...
    val cSeguridad : Boolean by masterViewModel.cSeguridad.observeAsState(initial = true)
    val cLimpieza : Boolean by masterViewModel.cLimpieza.observeAsState(initial = true)
    val cJardineria : Boolean by masterViewModel.cJardineria.observeAsState(initial = true)

    var readonlyAdministracion : Boolean = true
    var readonlySeguridad : Boolean = false
    var readonlyLimpieza : Boolean = false
    var readonlyJardineria: Boolean = false

    if(!(cSeguridad && cLimpieza && cJardineria)){
        readonlyAdministracion = true
        readonlySeguridad = !cSeguridad
        readonlyLimpieza = !cLimpieza
        readonlyJardineria = !cJardineria
    }
        ...
        LazyColumn(verticalArrangement = Arrangement.spacedBy(15.dp))
        {
            item {
                ...
                CardInfoServicios(modifier = Modifier.fillMaxWidth()) {
                    AdmGeneralv1 {
                        ...
                        numAdministracionGeneral(viewModel = viewModel, readonly = readonlyAdministracion, masterViewModel = masterViewModel)
                    }
                    GuardiasSegv1 {
                        GuardiasDeSeguridad()
                        numSeguridad(viewModel = viewModel, readonly = readonlySeguridad, masterViewModel = masterViewModel)
                    }
                    ServLimpiezav1 {
                        ServicioDeLimpieza()
                        numLimpieza(viewModel = viewModel, readonly = readonlyLimpieza, masterViewModel = masterViewModel)
                    }
                    ServJardineriav1 {
                        ServicioDeJardinerA()
                        numJardineria(viewModel = viewModel, readonly = readonlyJardineria, masterViewModel = masterViewModel)
                    }
            ...
            item {
                ...
                CardPrincipalPredio(modifier = Modifier.fillMaxWidth()) {
                    Predio()
                    inputPredio(viewModel = viewModel, masterViewModel = masterViewModel)
...
@Composable
fun numAdministracionGeneral (modifier: Modifier = Modifier, viewModel: SolicitarCotizacionViewModel, readonly: Boolean, masterViewModel: MasterViewModel){

    val numAdm: String by viewModel.numAdm.observeAsState("1")
    TextField(
        value = numAdm,
        onValueChange = { viewModel.onNumAdmChange(it)
                        masterViewModel.setCantAdmin(it)},
        textStyle = TextStyle(
            fontSize = 12.0.sp,
            color = Color(
                alpha = 255,
...
@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun numSeguridad (modifier: Modifier = Modifier, viewModel: SolicitarCotizacionViewModel, readonly: Boolean, masterViewModel: MasterViewModel){

    var inicial : String = "0"

    if(!readonly){
        inicial = "1"
    }

    val numGuardias: String by viewModel.numGuardias.observeAsState(inicial)


    TextField(
        value = numGuardias,
        onValueChange = { viewModel.onNumGuardiasChange(it)
                        masterViewModel.setCantSeguridad(it)},
        textStyle = TextStyle(
            fontSize = 12.0.sp,
            color = Color(
                alpha = 255,
...
@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun numLimpieza (modifier: Modifier = Modifier, viewModel: SolicitarCotizacionViewModel, readonly: Boolean, masterViewModel: MasterViewModel){
    ...
    val numLimp: String by viewModel.numLimpieza.observeAsState(inicial)

    TextField(
        value = numLimp,
        onValueChange = { viewModel.onNumLimpiezaChange(it)
                        masterViewModel.setCantLimpieza(it)},
        textStyle = TextStyle(
            fontSize = 12.0.sp,
            color = Color(
                alpha = 255,
...
@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun numJardineria (modifier: Modifier = Modifier, viewModel: SolicitarCotizacionViewModel, readonly: Boolean, masterViewModel: MasterViewModel){
    ...
    val numJardine: String by viewModel.numJardineria.observeAsState(inicial)

    TextField(
        value = numJardine,
        onValueChange = { viewModel.onNumJardineriaChange(it)
                        masterViewModel.setCantJardineria(it)
                        },
        textStyle = TextStyle(
            fontSize = 12.0.sp,
            color = Color(
...
@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun inputPredio(viewModel: SolicitarCotizacionViewModel, masterViewModel: MasterViewModel){
    var expanded by remember { mutableStateOf(false) }
    val items = viewModel.prediosResult.value
    ...
    DropdownMenu(
        ...
        items.forEach { predio ->
            DropdownMenuItem(onClick = {
                selectedText = predio.descripcion
                expanded = false
                viewModel.selectPredio(predio.id_predio)
                masterViewModel.selectPredio(predio)
            }) {
                Text(text = predio.descripcion)
            }
        }
    }
}