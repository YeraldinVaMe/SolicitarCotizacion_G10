...
import com.example.solicitarcotizacion.Network.ApiService
...
class RegisterViewModel: ViewModel(){
    ...
    fun agregarSolicitante(idPersona: Int){
        viewModelScope.launch(Dispatchers.IO) {
            try {
                ...
                withContext(Dispatchers.Main){
                    ...
                    result.onFailure {
                        ...
                        Log.e("ERROR1", "Error al registrar el solicitante"+result.toString())
                    }
                }
            }catch (e: Exception) {
                withContext(Dispatchers.Main) {
    ...
    fun agregarPersona(){
        ...
        viewModelScope.launch(Dispatchers.IO) {
            try {
                ...
                idPersona = personaUseCase.getIdPersona()!!
                Log.d("My",idPersona.toString())
                val result = personaUseCase.createPersonaUC(personaItem)
                withContext(Dispatchers.Main){
                    result.onSuccess {
                        Log.d("SUCCESS1", "Se agregó el persona")

                        agregarSolicitante(idPersona)
                        _showDialog.value = true
                        _showDialog2.value = false
                    }
                    result.onFailure {
                        _showDialog.value = false
                        _showDialog2.value = true
                        Log.e("ERROR1", "Error al registrar el persona")
                    }
                }
            }catch (e: Exception) {
                withContext(Dispatchers.Main) {
                    _showDialog.value = false
                    _showDialog2.value = true
                    Log.e("ERROR2", "Error al registrar el persona")
                }
            }
        }
    }
